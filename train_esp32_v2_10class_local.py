"""Local training script for the ESP32 V2 10-class SensorFusion-HAR model.

Runs the same pipeline as sensorfusion_har_ESP32.ipynb but optimized for local
CPU execution. Produces:
  - checkpoints_v2/best_sensorfusion_esp32_v2_10class.pt
  - exports/esp32_v2/sensorfusion_esp32_v2_10class.onnx
  - outputs/esp32_v2/summary_v2_10class.json
  - outputs/esp32_v2/v2_10class_confusion_f1.png
...
ESP32_ACTIVITY_LABELS = [
    ...
    "Jogging", "Cycling", "Ironing", "Vacuum Cleaning",
]

CHECKPOINT_DIR = "checkpoints_v2"
EXPORT_DIR = "exports/esp32_v2"
OUTPUT_DIR = "outputs/esp32_v2"
MODEL_TAG = "sensorfusion_esp32_v2_10class"
...
def build_merged_dataset(data_root: str = "data", include_mhealth: bool = True):
    print("Loading UCI-HAR, PAMAP2, and optional MHEALTH...")
...
def main():
    ...
    head_metrics = evaluate_model(model, test_loader, device, criterion)
    head_score = head_metrics["min_f1"] * 3.0 + head_metrics["macro_f1"] + head_metrics["acc"]
    torch.save({
        "model_state_dict": model.state_dict(),
        "prototype_state_dict": prototype_loss.state_dict(),
        "metrics": {
            "acc": head_metrics["acc"],
            "macro_f1": head_metrics["macro_f1"],
            "min_f1": head_metrics["min_f1"],
        },
        "labels": ESP32_ACTIVITY_LABELS,
    ...
    best_score, best_metrics = head_score, copy.deepcopy(head_metrics)
    history = []
    t0 = time.time()
    ...
    axes[0].set_title("V2 10-Class Confusion Matrix")
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(OUTPUT_DIR, "v2_10class_confusion_f1.png"), dpi=160, bbox_inches="tight")
    plt.close()
    ...
    with open(os.path.join(OUTPUT_DIR, "summary_v2_10class.json"), "w") as f:
