...
def load_model(checkpoint_path, device='cpu'):
    ...
    # FIX: training script lives under ./training/, not under
    # final_esp32_v2_useful11_package (that folder doesn't exist).
    ...
    num_classes = len(ckpt_labels) if ckpt_labels else state.get("num_classes", 11)
...
def direct_tflite_export(model, tflite_path):
    """Direct PyTorch -> TFLite was previously attempted via
    `from torch._export import tflite as tflite_exporter`. That submodule does
    not exist in any released PyTorch version; the call always raised
    AttributeError and the function silently returned None. The real
    deployment path is PyTorch -> ONNX -> onnx2tf -> TFLite, packaged in
    notebooks/sensorfusion_har_ESP32_v2_useful11_final.ipynb. This stub stays
    in place so the call site keeps working (it returns None and the ONNX
    fall-back kicks in), but no longer pretends a direct path exists.
    """
    return None
...
def generate_c_header(tflite_path, header_path, model_name="model_tflite"):
    ...
    # FIX: function previously assumed tflite_path was always a Path object,
    # but the caller passes a string in some code paths. Path() is idempotent
    # and accepts both, so this normalises without breaking either caller.
