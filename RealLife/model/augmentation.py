...
class SensorAugmentor:
    ...
    def __call__(self, x):
        ...
        # FIX: preserve numpy dtype too. The augmentations create float64
        # outputs by default (np.random.normal returns float64), which
        # silently upcasts a float32 input to float64. Downstream PyTorch
        # dataloaders then fail with "expected Float but got Double".
        if is_tensor:
        ...
        # restore the original numpy dtype (no-op if already correct)
        x_np = x_np.astype(np_dtype, copy=False)
