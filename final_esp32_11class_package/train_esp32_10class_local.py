"""Local training script for the ESP32 10-class SensorFusion-HAR model.

Runs the same pipeline as sensorfusion_har_ESP32.ipynb but optimized for local
CPU execution. Produces:
  - checkpoints/best_esp32_10class.pt
  - exports/esp32/sensorfusion_esp32_10class.onnx
  - outputs/esp32/summary_10class.json
  - outputs/esp32/10class_confusion_f1.png
...
def main():
    ...
    # ---------- Final eval ----------
    print("\n[Final evaluation]")
    ckpt = torch.load("checkpoints/best_esp32_10class.pt", map_location=device, weights_only=False)
    ...
    axes[0].set_title("10-Class Confusion Matrix")
    ...
    plt.tight_layout()
    plt.savefig("outputs/esp32/10class_confusion_f1.png", dpi=160, bbox_inches="tight")
    plt.close()
    ...
    with open("outputs/esp32/summary_10class.json", "w") as f:
    ...
    onnx_path = "exports/esp32/sensorfusion_esp32_10class.onnx"
    ...
    np.save("exports/esp32/calib_data_10class.npy", calib)
    print(f"Calibration data saved: exports/esp32/calib_data_10class.npy {calib.shape}")

    print("\nDone. Best checkpoint: checkpoints/best_esp32_10class.pt")
